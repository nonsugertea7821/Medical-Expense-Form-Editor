...
export default function ExplainDialog(): JSX.Element {
    const isDialogOpen = useRecoilValue(isDialogOpenSelector('explainDialog'));
    const [selectedSection, setSelectedSection] = useState('諸注意');

    const handleClose = useRecoilCallback(({ set }) => () => {
        set(isDialogOpenSelector(('explainDialog')), false);
    }, [isDialogOpenSelector(('explainDialog'))]);
                ...
                <Box sx={{ marginLeft: 2, flexGrow: 1, backgroundColor: 'white', borderRadius: '3px', padding: '40px' }}>
                    ...
                    {selectedSection === '操作方法' && (
                        <DialogContentText component="div" sx={{ marginBottom: "20px" }}>
                            ...
                            <p>入力データ確認：入力したデータを確認できます。</p>
                            <p>機能</p>
                            <ul>
                                <li>ダウンロード:提出用ファイルをダウンロード出来ます。</li>
                                <li>中断:セーブデータをJson形式で書き出し/読み込みが出来ます。</li>
                                <li>オールクリア：全ての入力情報とキャッシュが削除されます。</li>
                            </ul>
                        </DialogContentText>
                    )}
